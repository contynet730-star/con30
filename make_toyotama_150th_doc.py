...
# ② 歴史（X3：資料に基づく史実版／校名は郡名由来＋学田の物語／144字）
para(
    "　本校は明治九年十一月、南蔵院の寺子屋を母体に、教員二名・児童八十名で"
    "開校されました。校名「豊玉」は学区がまたがっていた「北豊島郡」と「東多摩郡」"
    "に由来し、地域の方々は「学田」を耕してその収穫を学校に充てるなど、当初より"
    "教育を支えてこられました。以来一万二千六百六十八名の卒業生を数えます。",
    size=11
)
...
# 文字数注記（資料用・小さく）
para(
    "（文字数：全体591文字・600文字以内 ／ 史実出典：150周年行事関係資料）",
